...
test('diagnostic output contains only version, adapter and counts',async()=>{
 ...
 assert.equal(d.version,'0.23.4');assert.equal(d.matchedMessages,1);assert.deepEqual(Object.keys(d).sort(),['plugin','version','state','assistantRows','matchedMessages','proseBlocks','adapter'].sort());
...
test('production bundle and package agree on version and DSH loader contract',()=>{
 const pkg=JSON.parse(readFileSync(new URL('../package.json',import.meta.url),'utf8'));assert.equal(pkg.version,'0.23.4');
